...
class Simulator(ABC):
    ...
    def step(self, xt: torch.Tensor, t: torch.Tensor, h: torch.Tensor, **kwargs) -> torch.Tensor:
        ...
        Args:
            xt (torch.Tensor): Current state at time t.
                Shape: (batch_size, dim) for vectors
                   or  (batch_size, c, h, w) for images
            t (torch.Tensor): Current time.
                Shape: () or (1,) or (batch_size, 1, 1, 1)
            h (torch.Tensor): Step size (dt).
                Shape: () or (1,) or (batch_size, 1, 1, 1)
    ...
    def simulate(self, x: torch.Tensor, ts: torch.Tensor, **kwargs) -> torch.Tensor:
        ...
        Args:
            ...
            ts (torch.Tensor): Time discretization.
                Shape: (num_timesteps,) or (batch_size, num_timesteps, 1, 1, 1)
    ...
    def simulate_with_trajectory(self, x: torch.Tensor, ts: torch.Tensor, **kwargs) -> torch.Tensor:
        ...
        Returns:
            torch.Tensor: Full trajectory over time.
                Shape: (batch_size, num_timesteps, dim) or (batch_size, num_timesteps, c, h, w)
        ...
        if ts.ndim == 1:
            ...
            for t_idx in tqdm(range(ts.shape[1] - 1), desc="Simulating"):
                t = ts[:, t_idx]
                h = ts[:, t_idx + 1] - ts[:, t_idx]
                x = self.step(x, t, h, **kwargs)
                xs.append(x.clone())
...
class Sampleable(ABC):
    ...
    Lab03 needs to optionally return labels for conditional generation.
    To stay backward compatible, we return a tuple ``(samples, labels)``
    where labels can be ``None`` for unconditional cases.
    """
    ...
    def sample(self, num_samples: int) -> Tuple[torch.Tensor, Optional[torch.Tensor]]:
        ...
        Returns:
            Tuple[torch.Tensor, Optional[torch.Tensor]]: samples and optional labels
                - samples: shape (num_samples, dim) or (num_samples, c, h, w)
                - labels: shape (num_samples,) or None when unlabeled
        """
        pass