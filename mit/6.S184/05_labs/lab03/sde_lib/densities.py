...
class IsotropicGaussian(nn.Module, Sampleable):
    """
    Isotropic Gaussian sampler for arbitrary tensor shapes.
    
    Generates i.i.d. Gaussian noise N(0, σ²I) with a specified shape.
    Commonly used as the source distribution p_simple(x) in flow matching.
    
    For lab03 (MNIST images), typical usage:
        IsotropicGaussian(shape=[1, 32, 32], std=1.0)
    
    which samples noise of shape (bs, 1, 32, 32) matching MNIST's format.
    
    Args:
        shape (List[int]): Shape of a single sample (without batch dimension).
            Examples:
                - [2] for 2D vectors
                - [1, 28, 28] for grayscale 28×28 images  
                - [3, 64, 64] for RGB 64×64 images
        std (float): Standard deviation σ of the Gaussian. Default: 1.0
    
    Returns from sample():
        Tuple[torch.Tensor, None]:
            - samples: shape (num_samples, *shape)  — i.i.d. Gaussian noise
            - labels: None  — no class labels (unconditional noise)
    """
    ...
    def __init__(self, shape, std: float = 1.0):
        ...
        self.shape = shape
        self.std = std
        # Dummy buffer to track device (moves with .to(device))
        self.dummy = nn.Buffer(torch.zeros(1))

    def sample(self, num_samples: int):
        """
        Sample i.i.d. Gaussian noise.
        ...
        Returns:
            Tuple[torch.Tensor, None]:
                - samples: shape (num_samples, *self.shape)
                - labels: None
        """
        samples = self.std * torch.randn(num_samples, *self.shape, device=self.dummy.device)
        return samples, None


class MNISTSampler(nn.Module, Sampleable):
    """
    Sampleable wrapper for the MNIST handwritten digit dataset.
    
    Downloads and loads MNIST training data with preprocessing:
        1. Resize to (32, 32) from original (28, 28)
        2. Convert PIL image to torch.Tensor in [0, 1]
        3. Normalize to [-1, 1] via (x - 0.5) / 0.5
    
    This provides the target data distribution p_data(z, y) where:
        - z: grayscale images, shape (1, 32, 32), range [-1, 1]
        - y: digit labels, shape (), values in {0, 1, ..., 9}
    
    Used as the conditioning source in lab03 CFG training.
    
    Returns from sample():
        Tuple[torch.Tensor, torch.Tensor]:
            - samples: shape (num_samples, 1, 32, 32)  — normalized MNIST images
            - labels:  shape (num_samples,)            — integer labels 0-9
    """
    ...
    def __init__(self):
        ...
        # Dummy buffer to track device
        self.dummy = nn.Buffer(torch.zeros(1))

    def sample(self, num_samples: int):
        """
        Sample random MNIST images with labels.
        
        Args:
            num_samples (int): Number of (image, label) pairs to sample.
        
        Returns:
            Tuple[torch.Tensor, torch.Tensor]:
                - samples: shape (num_samples, 1, 32, 32), dtype float32
                - labels:  shape (num_samples,), dtype int64
        
        Raises:
            ValueError: If num_samples > 60,000 (MNIST training set size).
        """
        ...
        labels = torch.tensor(labels, dtype=torch.int64, device=self.dummy.device)
...
class Gaussian(torch.nn.Module, Sampleable, Density):
    ...
    def isotropic(cls, dim: int, std: float = 1.0) -> "Gaussian":
        """
        Create an isotropic Gaussian distribution N(0, std²I).
        ...
        Returns:
            Gaussian: Centered Gaussian with diagonal covariance.
        """
        return cls(
    ...
    def log_density(self, x: torch.Tensor) -> torch.Tensor:
        ...
        """
        return rearrange(self.distribution.log_prob(x), 'b -> b 1')
    ...
    def sample(self, num_samples: int):
        ...
        Returns:
            Tuple[torch.Tensor, Optional[torch.Tensor]]: samples and None labels.
        ...
        """
        return self.distribution.sample((num_samples,)), None
...
class GaussianMixture(torch.nn.Module, Sampleable, Density):
    ...
    def sample(self, num_samples: int):
        ...
        Example:
            ...
            >>> samples, _ = gmm.sample(5000)
            >>> # Visualize: samples cluster in 5 groups
            >>> plt.scatter(samples[:, 0], samples[:, 1], alpha=0.5)
        """
        return self.distribution.sample(torch.Size((num_samples,))), None
    ...
    def symmetric_2D(
        ...
        means = rearrange([torch.cos(angles), torch.sin(angles)], 'c n -> n c') * scale
        # Spherical covariances
        covs = torch.diag_embed(torch.ones(nmodes, 2) * (std ** 2))
        # Equal weights (normalized automatically)
        inv_nmodes = 1.0 / nmodes
        weights = torch.ones(nmodes) * inv_nmodes
        return cls(means, covs, weights)


class CirclesSampleable(Sampleable):
    """
    Two concentric circles sampleable distribution (from sklearn).
    
    This is a common toy dataset for evaluating generative models.
    
    Args:
        device (str or torch.device): Target device.
        noise (float): Noise level added to circle points.
        scale (float): Scaling factor for coordinates.
        offset (torch.Tensor, optional): 2D offset applied to samples.
    """
    
    def __init__(self, device, noise: float = 0.05, scale: float = 5.0, offset: torch.Tensor = None):
        self.noise = noise
        self.scale = scale
        self.device = device
        if offset is None:
            offset = torch.zeros(2, device=device)
    ...
    def sample(self, num_samples: int):
        ...
        samples = self.scale * torch.from_numpy(samples).to(dtype=torch.float32, device=self.device) + self.offset
        return samples, None


class MoonsSampleable(Sampleable):
    """
    Two moons sampleable distribution (from sklearn).
    
    Args:
        device (str or torch.device): Target device.
        noise (float): Noise level added to moon points.
        scale (float): Scaling factor for coordinates.
        offset (torch.Tensor, optional): 2D offset applied to samples.
    """
...
class CheckerboardSampleable(Sampleable):
    """
    Checkerboard sampleable distribution.
    
    Generates points uniformly within a checkerboard pattern on [-scale, scale]^2
    using rejection sampling.
    
    Args:
        device (str or torch.device): Target device.
        grid_size (int): Number of squares along each axis.
        scale (float): Half-width of the square domain.
    """
    
    def __init__(self, device, grid_size: int = 3, scale: float = 5.0):
    ...
    def sample(self, num_samples: int):
        ...
        samples = torch.zeros(0, 2, device=self.device)
        while samples.shape[0] < num_samples:
            new_samples = (torch.rand(num_samples, 2, device=self.device) - 0.5) * 2 * self.scale
