...
class CFGVectorFieldODE(ODE):
    """
    ODE wrapper for classifier-free guidance (CFG) with learned vector field.
    
    Implements the guided vector field:
        ũ_t(x|y) = (1 - w)·u_t(x|∅) + w·u_t(x|y)
    
    where:
        - u_t(x|y)  : conditional vector field (class y)
        - u_t(x|∅)  : unconditional vector field (null label ∅ = 10)
        - w         : guidance scale (w > 1 strengthens conditioning)
    
    This guided field is used at inference time to sample from p(x|y)
    with enhanced conditioning strength.
    
    Args:
        net: Neural network u_t^θ(x|y) (must be nn.Module)
        guidance_scale (float): Guidance weight w. Typical values: 1.0–7.0
            - w = 1.0: No guidance (standard conditional generation)
            - w > 1.0: Stronger conditioning (sharper, more class-specific)
            - w = 0.0: Fully unconditional
    
    Note:
        This class does NOT inherit nn.Module because it doesn't contain
        learnable parameters. The `net` is already a trained nn.Module.
        Device management (.to(device)) should be done on `net` before
        passing it to CFGVectorFieldODE.
    """
    ...
    def drift_coefficient(self, x: torch.Tensor, t: torch.Tensor, y: torch.Tensor, **kwargs) -> torch.Tensor:
        """
        Compute the CFG-guided drift coefficient.
        
        Args:
            x (torch.Tensor): Current state (noisy image).
                Shape: (bs, c, h, w)
            t (torch.Tensor): Current time.
                Shape: (bs, 1, 1, 1)
            y (torch.Tensor): Class labels for conditioning.
                Shape: (bs,)  — integer labels in {0, 1, ..., 9}
        
        Returns:
            torch.Tensor: Guided vector field ũ_t(x|y).
                Shape: (bs, c, h, w)
        """
        # Conditional vector field: u_t(x|y)
        guided_vector_field = self.net(x, t, y)
        
        # Unconditional vector field: u_t(x|∅) where ∅ = 10
